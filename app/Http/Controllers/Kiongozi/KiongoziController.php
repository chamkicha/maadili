...
class KiongoziController extends Controller
{
    ...
    public function ajiraTaarifa(Request $request)
    {
        ...
        $latestData = Sectiontaarafa478::where('user_id','=',auth()->user()->id)->latest()->first();
            ...
            $file_number = $this->file_number($request);
                ...
                $isactive=true;
            $kiongozi = Sectiontaarafa478::updateOrCreate(['user_id' => auth()->user()->id], [
            ...
            \Log::info('AjiraPayload: ' . json_encode($kiongozi));
            if( $kiongozi){
                ...
                $update_title_id = User::where('id','=',auth()->user()->id)->update(['title_id' => $request->title_id]);
        ...
        public function updateAjiraTaarifa(Request $request ,$token)
        {
            ...
            if ($validator->fails()) {
                \Log::error('AjiraUpdateError: ' . $validator->errors());
                return response()->json($validator->errors());
            }
            ...
            \Log::info('AjiraPayloadUpdate: ' . json_encode($kiongozi));

            $createMenuLookup = createMenuLookup('stage_three');
            $update_title_id = User::where('id','=',auth()->user()->id)->update(
                [
    ...
    public function file_number($request)
    {
        ...
        // Retrieve Kanda (Region) abbreviation
        $kanda = Zone::join('regions', 'regions.zone_id', '=', 'zones.id')
                    ->where('regions.id', $request->mkoa_sasa)
                    ->first();
        $kanda = $kanda ? $kanda->abbreviation : 'HQ';
        
        // Retrieve Taasisi (Employer) abbreviation
        $taasisi = Office::where('id', $request->employer)->first();
        $taasisi = $taasisi ? $taasisi->abbreviation : 'PPRA';
        
        // Retrieve Cheo (Title) abbreviation
        $cheo = Title::where('id', $request->title_id)->first();
        $cheo = $cheo ? $cheo->abbreviation : 'MNGR';
        
        // Static CPF string
        $CPF = 'CPF';
        
        // Generate the file number prefix
        $file_number_prefix = 'ES' . '/' . $kanda . '/' . $CPF . $taasisi . '/' . $cheo;
        
        do {
            $namba = mt_rand(100000, 999999);
        } while (User::where('file_number', $namba)->exists());
        
        // Generate the complete file number
        $file_number = $file_number_prefix . '/' . $namba;

        // Proceed with updating the user record with the new file number
        $user->file_number = $file_number;
        $user->save();
